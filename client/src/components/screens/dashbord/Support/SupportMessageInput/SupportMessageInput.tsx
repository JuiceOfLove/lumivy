...
import styles from "./SupportMessageInput.module.css";
...
const SupportMessageInput: React.FC<Props> = ({
  ticketId,
  replyTo,
  onCancelReply,
}) => {
  ...
  const sendPlain = (e: FormEvent) => {
    e.preventDefault();
    if (!text.trim()) return;
    SupportSocketService.send(text.trim(), ticketId, replyTo?.id);
    setText("");
    onCancelReply();
  };
